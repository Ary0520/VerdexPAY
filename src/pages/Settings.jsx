...
function Row({ icon, label, value, onClick, children, border = true, danger }) {
  return (
    <div onClick={onClick} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '15px 20px', borderBottom: border ? `1px solid ${C.border}` : 'none', cursor: onClick ? 'pointer' : 'default', transition: 'background-color 0.12s' }}
    ...
    >
      ...
      {children}
      {onClick && !children && <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M6 4L10 8L6 12" stroke={C.sub} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>}
    </div>
  )
}
...
function Toggle({ on, onChange }) {
  return (
    <button onClick={e => { e.stopPropagation(); onChange(!on) }} style={{ width: 44, height: 26, borderRadius: 9999, border: 'none', cursor: 'pointer', backgroundColor: on ? C.green : C.border, position: 'relative', padding: 0, flexShrink: 0, transition: 'background-color 0.2s' }}>
          ...
          <Section title="Wallet & Payments">
            <div style={{ padding: '15px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontFamily: 'Inter', fontSize: 11, color: C.sub, marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{walletAddress ?? 'Loading...'}</div>
              </div>
